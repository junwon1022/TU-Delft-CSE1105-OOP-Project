...
public class UserPreferencesTest {
    ...
    void testAddBoard() throws JsonProcessingException {
        ...
        PreferencesBoardInfo board = new PreferencesBoardInfo(b.title, b.key, b.password, b.font, b.colour);
    ...
    @Test
    void testAddBoard_nullBoard() throws JsonProcessingException {
        UserPreferences prefs = Mockito.mock(UserPreferences.class);
    ...
    @Test
    void testUpdateBoardTitle_inList() throws JsonProcessingException {
        UserPreferences prefs = Mockito.mock(UserPreferences.class);
        ...
        PreferencesBoardInfo board1 = new PreferencesBoardInfo(b1.title, b1.key, b1.password, b1.font, b1.colour);
        PreferencesBoardInfo board2 = new PreferencesBoardInfo(b2.title, b2.key, b2.password, b2.font, b2.colour);

        // mock the behavior of the UserPreferences object
        Mockito.when(prefs.updateBoardTitle(Mockito.anyString(), Mockito.any(PreferencesBoardInfo.class), Mockito.anyString())).thenReturn(board2);
        Mockito.when(prefs.getBoards(Mockito.eq("http://localhost:8080/"))).thenReturn(List.of(board1, board2));

        // call the updateBoardTitle method with board1 and a new title
        PreferencesBoardInfo updatedBoard = prefs.updateBoardTitle("http://localhost:8080/", board1, "new title");
